			...
			if (sel == 1 && log != -1) {
				...
				while (true) {
					...
					if (input == 1) {
						...
						System.out.print("[삭제] 삭제 수량 입력 : ");
						int delQty = scan.nextInt();

						for (int i = 0; i < count; i++) {
							if (idList[log].equals(cartList[i][0]) && delItem.equals(cartList[i][1])) {
								for (int k = 0; k < items.length; k++) {
									if (delItem.equals(items[k]) && qty[k] >= delQty) {
										...
										delQty--;
										break;
									} else if (qty[k] < delQty) {
										System.err.println("장바구니 갯수 초과");
										i = count;
										break;
									} else if (qty[k] == 0) {
										i = count;
